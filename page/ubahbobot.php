...
$querylihat="SELECT id_jenistema,bobot,id_bobotkriteria,kriteria.namaKriteria AS namaKriteria FROM bobot_kriteria INNER JOIN kriteria USING(id_kriteria) WHERE id_jenistema='$id'";
...
<form id="form" action="./proses/prosesubah.php" method="POST">
    ...
    <div class="panel-middle">
        <div class="group-input">
            <div class="group-input">
                <?php
                $query="SELECT nama_tema FROM jenis_tema WHERE id_jenistema='$id'";
                $execute=$konek->query($query);
                $data=$execute->fetch_array(MYSQLI_ASSOC);
                ?>
                <div class="group-input">
                    <label for="jenisbarang">Jenis Bobot Tema</label>
                    <input class="form-custom" value="<?php echo $data['nama_tema'];?>" disabled type="text" autocomplete="off" required name="barang" id="barang">
                </div>
            </div>
        </div>
        <?php
